...
def words_graph():
    ...
    Words = r"C:\Users\bbenx\Dropbox\projects\CSCI-4470-OpenSource-master\Lab6\words_dat.txt.gz"
    fh = gzip.open( Words, 'r')
    words = set()
...
def words_graph():
    """Return the words example graph from the Stanford GraphBase"""
    Words = r"C:\Users\bbenx\Dropbox\projects\CSCI-4470-OpenSource-master\Lab6\words_dat.txt.gz"
    WordsFour = r"C:\Users\bbenx\Dropbox\projects\CSCI-4470-OpenSource-master\Lab6\words4_dat.txt.gz"
    fh = gzip.open( WordsFour, 'r')
    words = set()
    for line in fh.readlines():
        line = line.decode()
        if line.startswith('*'):
    ...
    def edit_distance_one(word):
        ...
        for pword in list(p):
            ...
            for i in range(len(PermWord)):
                left, c, right = PermWord[0:i], PermWord[i], PermWord[i + 1:]
                j = lookup[c]  # lowercase.index(c)
                for cc in lowercase[j + 1:]:
                    yield left + cc + right
    candgen = ((word, cand) for word in sorted(words)
